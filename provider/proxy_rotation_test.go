...
// TestProxyAddRotatesCredentials verifies that proxyAdd purges an existing
// same-address entry with different inline credentials when a new one is added,
// so a re-paste becomes a rotation instead of a silent duplicate that the
...
// TestProxyReloader_ReloadRotationExecution verifies that reload()
// cancels a running proxy and spawns a new one when credentials change.
