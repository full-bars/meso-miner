...
func TestEnableProfilingWithRetryBindsOnceParentReleases(t *testing.T) {
	...
	go func() {
		enableProfilingWithRetry(context.Background(), addr, connect.EnableProfiling, 5*time.Second, 25*time.Millisecond, sink.logf)
		close(done)
	}()
	select {
	case <-done:
...
func TestEnableProfilingWithRetryGivesUpWhilePortStaysBusy(t *testing.T) {
	...
	var sink logSink
	start := time.Now()
	enableProfilingWithRetry(context.Background(), parent.Addr().String(), connect.EnableProfiling, 150*time.Millisecond, 20*time.Millisecond, sink.logf)
...
// A non-loopback address will never start working, so it is reported once,
// not retried.
func TestEnableProfilingWithRetryDoesNotRetryPermanentErrors(t *testing.T) {
	calls := 0
	var sink logSink
	enableProfilingWithRetry(context.Background(), "0.0.0.0:6060", func(a string) error {
		calls++
		return connect.EnableProfiling(a)
...
func TestEnableProfilingWithRetryStopsOnContextCancel(t *testing.T) {
	...
	go func() {
		enableProfilingWithRetry(ctx, parent.Addr().String(), connect.EnableProfiling, time.Minute, 20*time.Millisecond, func(string, ...any) {})
		close(done)
	}()
