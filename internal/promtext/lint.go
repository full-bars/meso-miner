// Package promtext checks /metrics output against the Prometheus classic
// text exposition format. The provider writes that format by hand with no
// client library, so tests use this to catch what Prometheus would reject
// (or silently misread) before a release ships.
package promtext
...
var classicTypes = map[string]bool{
...
func splitSample(line string) (name, labels, rest string, err error) {
	...
	for {
		...
		lname := line[i : i+eq]
		if !labelName.MatchString(lname) {
