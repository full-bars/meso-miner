...
4. After the candidate's ACK, the parent calls `yieldCoordinatorSession()` (`provider/hotswap.go`), which runs every registered closer at once. Each proxy client registers `platformTransport.Close()` as its closer (`provider/main.go`). This is the mass drop.
