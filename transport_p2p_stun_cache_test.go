...
func TestSTUNHealthyAfterTTL(t *testing.T) {
	...
	// After returning healthy, the expired entry should have been purged.
	if _, ok := stunFailureCache.Load(url); ok {
		t.Fatal("expired entry should be absent from cache after health check")
	}
}
