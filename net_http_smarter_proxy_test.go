...
// ---------------------------------------------------------------------------
// TestSerialRecencyTiebreak: two dialers with same priority and same success
// rate but different lastSuccessTime — the more recent one must sort first.
// ---------------------------------------------------------------------------

func TestSerialRecencyTiebreak(t *testing.T) {
	now := time.Now()

	// Both priority 0, both 8/10 = 80% success rate, but different recency.
	dialers := []*clientDialer{
		{
			priority:        0,
			successCount:    8,
			errorCount:      2,
			lastSuccessTime: now.Add(-10 * time.Minute), // old
		},
		{
			priority:        0,
			successCount:    8,
			errorCount:      2,
			lastSuccessTime: now.Add(-1 * time.Minute), // recent
		},
	}
	...
	if !dialers[0].lastSuccessTime.After(dialers[1].lastSuccessTime) {
		t.Errorf("more recent dialer should sort first: dialers[0].last=%v dialers[1].last=%v",
			dialers[0].lastSuccessTime, dialers[1].lastSuccessTime)
	}
}
