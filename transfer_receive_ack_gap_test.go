...
func TestReceiveSequenceGapWakeWritesProvableHoleEarly(t *testing.T) {
	synctest.Test(t, func(t *testing.T) {
		...
		startTime := time.Now()
		sequence.readAcks(t, 500*time.Millisecond, sequence.updateSelectiveRun(2, 3), "provable hole")
		if elapsed := time.Since(startTime); elapsed != 0 {
			t.Fatalf("provable hole was written %s into the compression interval, want at once", elapsed)
		}
	})
}

// When a hole fills, the sender's flight is head blocked until it sees the
// cumulative head ack that passed it. The gap wake ends the compression wait
// at once so the head ack leaves immediately, even though a proof of the same
